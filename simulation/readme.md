...
- [Security Considerations](#security-considerations)
  - [Key Rotation for Authentication](#key-rotation-for-authentication)
    - [Implementation Details](#implementation-details)
      ...
      - [Complex Key Rotation with Diffie-Hellman Group 14](#complex-key-rotation-with-diffie-hellman-group-14)
...
- **Soft Key Rotation:**
  ...
  2. The new key is then encrypted with the existing key and transmitted over TCP.
...
- **More Complex Key Rotation with Diffie-Hellman Group 14:**
  ...
  4. A portion of the challenge is used as a seed for a random function, introducing an additional layer of complexity to deter potential man-in-the-middle attacks.
- **Impact:** During key updates, data transmission is temporarily halted to prevent authentication failures. Different communication channels respond variably; some abort the ongoing transmission, while others allow the package to send until it is authenticated. 
Got it! I've made the necessary adjustments to reflect that the TCP connections are protected with Modbus authentication. Here's the revised section:
...
**Receiver's Response:**
- On the receiver side, in case of an authentication failure, the system rejects the incoming data, logs the failure, and awaits the next package. 
