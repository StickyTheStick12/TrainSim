...
#### 6. **JSONs Subdirectory**
   - **data.json:** Used by Flask to display information generated by the simulation. This file also stores Flask logs and is retained between program runs.

   - **arrival.json and departure.json:** Generated during runtime, these files are protected by HMAC-SHA256. They serve as temporary storage for arrivals and departures information received from Trafikverket and are deleted when the program is executed.

   - **usercredentials.json:** Contains login credentials for the Flask website.
...
1. **Daytime Operation:**
   - The simulation is designed to operate during the daytime when train arrivals and departures are expected. As a result, the program may not work as expected during evenings or when there are long periods without arriving trains.
